...
# Íconos de OPERACIÓN del menú de Depósito. Mismo set que los de sector
# —heroicons outline, viewBox 24, stroke 1.5— y un diccionario aparte a
# propósito: SECTORES contesta "¿de qué área es esta pantalla?" y esto
# contesta "¿qué hace este botón?". Juntarlos haría que un ícono nuevo de
# operación apareciera en la barrita de navegación, que es otro lugar.
#
# `aria-hidden` porque el rótulo va AL LADO y dice lo mismo: un lector de
# pantalla que los lea a los dos repite cada botón.
_ICONOS_DEPOSITO = {
    "retirar": (
    ...
    "pase": (
        '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5"/></svg>'
    ),
    "reingreso": (
...
def _filas_de_que_comprar(
    ...
    *, kilajes: dict | None = None, en_camino: dict | None = None,
    ...
    `aportes` es una entrada por CARGA —{"etiqueta": "Día 26/09", "pide":
    {articulo_id: total}}— y cada `pide` es YA lo que esa carga mostró, con
    su margen puesto donde corresponde (`lo_que_pide_la_carga`). Acá no se
    aplica ningún margen: el del listado se fue el 23/09, porque dos
    márgenes que se multiplican son invisibles (20% y 10% son 32%).

    LO ÚNICO QUE SE SUMA ES EL MISMO ARTÍCULO ENTRE CARGAS: dos fechas de
    Día y una de Tailem son una fila de tomate. Nadie suma Mango con Cherry.

    LA MAGNITUD DE LA FILA ES LA DEL ARTÍCULO (`unidades`, que sale de
    `_unidad_de_cada_articulo`), la misma en la que la carga se tipeó. Un
    artículo cuya unidad no se puede saber —lo que la alerta
    `unidades_que_difieren` señala— deja la fila sin número: su total está
    en una unidad y el piso se mediría en otra, sin descuadrar nada.
    ...
    for aporte in aportes:
        for articulo_id, total in aporte["pide"].items():
            if total is None:
                continue
            por_articulo.setdefault(articulo_id, []).append((aporte["etiqueta"], float(total)))

    filas = []
    for articulo_id, de_quien in por_articulo.items():
        articulo = articulos.get(articulo_id) or {}
        unidad = unidades.get(articulo_id)
        pide = sum(total for _e, total in de_quien) if unidad else None

        del_piso = piso.get(articulo_id) or {}
        lo_comprado = comprado.get(articulo_id) or {}
        comprado_magnitud = (
            lo_comprado.get("kilos") if unidad == "kilo" else lo_comprado.get("conteo")
        )
...
def _contexto_de_que_comprar(request: Request, aviso: str | None = None):
    ...
    try:
        cargas = cargas_con_renglones(borrador["cargas"])
        aportes = [
            {
                "etiqueta": f"{carga['cliente_nombre']} {carga['fecha'].strftime('%d/%m')}",
...
def _propuesto_de_la_carga(carga: dict) -> dict:
    """Lo que el promedio propone para una carga, CRUDO y sin margen. Vacío en "a mano".

    EL MODO SE DECIDE ACÁ UNA VEZ, para las dos pantallas que lo necesitan
    —la carga y el listado—. Escrito en cada una, la copia que se olvide del
    modo le sumaría el promedio a una carga "a mano": el listado compraría lo
    tipeado MÁS lo que el cliente suele pedir, sin que nada se vea raro.
    """
    if carga["modo"] == "manual":
        return {}
    return _promedio_por_articulo(carga["cliente_id"], carga["promedio_anterior_a"])
...
def _articulos_para_cargar(cliente_id: int,
    ...
    # LO QUE EL CAMPO MUESTRA SALE DE `lo_que_pide_la_carga`, que es la misma
    # función con la que el listado la suma: el margen solo sobre lo propuesto,
    ...
    filas = []
    for a in articulos:
        # EL POR BULTO DE LA FILA: el que se declaró en esta carga, y si no se
        # declaró ninguno, el de la ficha del cliente (dueño, 23/09: "cargar
        # indistintamente kilos totales, bultos y kilos por bulto").
        ficha = contenidos.get(a["id"])
        contenido = declarado.get(a["id"]) or ficha
        total = pide.get(a["id"])
        filas.append({
            "articulo_id": a["id"],
            "nombre": a["nombre"],
            ...
            # LA MISMA PALABRA QUE QUÉ COMPRAR HOY ("kg por bulto"): la carga y
            # el listado nombran igual el mismo número.
            "palabra": PALABRA_DE_LA_UNIDAD.get(unidades.get(a["id"]), ""),
            "total": total,
            ...
            "contenido": contenido,
            "contenido_ficha": ficha,
            "por_bulto_declarado": a["id"] in declarado,
...
def _contexto_de_carga(carga: dict, aviso: str | None = None) -> dict:
    ...
    propuesto = _propuesto_de_la_carga(carga)
    return {
...
async def guardar_carga_de_compra_ruta(request: Request, cliente_id: int, fecha: str):
    ...
    try:
        ...
        renglones, por_bulto = _renglones_del_formulario(formulario, cliente_id)
        # EN "A MANO" EL CAMPO DEL MARGEN NO SE DIBUJA —no mueve nada: el
        # margen va solo sobre lo propuesto— así que no llega. Leerlo igual
        # con `margen_valido` lo pisaría con el sugerido, y la carga volvería
        # a "del promedio" con un margen que nadie eligió.
        if "margen" in formulario:
...
def _renglones_del_formulario(formulario, cliente_id: int) -> tuple[dict, dict]:
    """({articulo_id: total en la MAGNITUD}, {articulo_id: por bulto declarado}).

    TRES CAMPOS POR FILA Y UNO MANDA (dueño, 23/09): kilos en total, bultos y
    kilos por bulto, relacionados en la pantalla. Se guarda el TOTAL, que es
    lo que el listado suma, y el POR BULTO; los bultos salen de dividir.

      - `total_` manda si llegó con un número.
      - Si no, `bultos_ × por bulto` — el por bulto tipeado, o el de la ficha.
        Es el camino sin JavaScript, y el de un formulario viejo.
      - Sin total y sin por bulto no hay nada que guardar: los bultos solos no
        dicen cuántos kilos son.
    ...
    cambie; NULL es exactamente "proponé el de la ficha".

    Y EL CONTENIDO DE LA FICHA SALE DEL SERVER, no de un campo escondido: con
    él viajando en el formulario, un POST armado a mano cambiaría la
    ...
    TEXTOS del total tal como la pantalla los dibujó, no entre floats. Cambiar
    solo el por bulto SÍ cuenta como corrección.
    """
    campos = dict(formulario.multi_items())
    contenidos = _bulto_de_cada_articulo(cliente_id)
    articulos = set()
    for clave in campos:
        for prefijo in ("total_", "bultos_"):
            if clave.startswith(prefijo) and clave[len(prefijo):].isdigit():
                articulos.add(int(clave[len(prefijo):]))

    renglones, por_bulto = {}, {}
    for articulo in sorted(articulos):
        ficha = contenidos.get(articulo)
        tipeado = _numero_del_formulario(campos.get(f"por_bulto_{articulo}", ""))
        declarado = tipeado if tipeado is not None and tipeado > 0 else None
        contenido = declarado or ficha

        total = _numero_del_formulario(campos.get(f"total_{articulo}", ""))
        if total is None or total <= 0:
            bultos = _numero_del_formulario(campos.get(f"bultos_{articulo}", ""))
            if bultos is None or bultos <= 0 or not contenido:
                continue
            total = bultos * contenido

        cambio_el_bulto = declarado is not None and (ficha is None or abs(declarado - ficha) > 1e-9)
        propuesto = campos.get(f"propuesto_{articulo}")
        if (propuesto is not None and not cambio_el_bulto
                and str(campos.get(f"total_{articulo}", "")).strip() == propuesto):
            continue
        renglones[articulo] = total
        if cambio_el_bulto:
            por_bulto[articulo] = declarado
    return renglones, por_bulto
...
def ver_que_comprar(request: Request):
    """Qué comprar hoy: cuántos cajones de cada artículo, sumando las cargas elegidas.
...
async def guardar_que_comprar(request: Request):
    ...
    cargas = {int(v) for v in formulario.getlist("carga") if str(v).strip().isdigit()}
    kilajes = {}
...
# retira. Acá el cajón llega CON la mercadería. No comparten una sola tabla.
# ============================================================================
...
def ver_deposito(request: Request, aviso: str | None = None):
    """Hub del área Depósito, en tres recuadros: Mercadería, Pedidos y Stock.

    LAS SEIS DE STOCK ESTÁN ACÁ y ya no detrás de un hub intermedio
    (`/deposito/stock`, borrado el 20/09): son las operaciones que más se
    tocan y cada una costaba un toque de más. El recuadro con título hace el
    trabajo que hacía esa pantalla del medio —agrupar y nombrar— sin cobrar
    la navegación.
...
def _porcion_elegida(articulo_id: str, que_conto: str) -> tuple[dict | None, bool, str | None]:
    """Traduce "sueltos" / "segunda" / id de ficha a (ficha, es_segunda, error).

    UNA SOLA COPIA, y no es cosmético: la eligen el Stock Físico y la Merma,
    y son la MISMA pregunta —de qué porción del artículo estamos hablando—.
    Escrita dos veces, un día una acepta una ficha de otro artículo y la
    otra no, y la que decide es la que el operario haya usado ese día.

    Que la ficha sea DEL ARTÍCULO se valida siempre: una ficha de otro
    artículo no es un dato raro, es un dato roto. En la merma además lo
    rechaza la base (FK compuesta `movimientos_stock_ficha_del_articulo`),
    así que acá el chequeo es para dar un cartel entendible, no para
    sostener la regla — la regla vive donde se escribe.
    """
    elegido = que_conto.strip()
    if not elegido:
        return None, False, "Elegí de qué se trata: los bultos sueltos, la segunda o las cajas de una ficha."
    if elegido == "segunda":
    ...
    if ficha is None:
        return None, False, "Esa ficha no es de este artículo."
...
def _renderizar_pantalla_merma(
    ...
    """La pantalla de merma, con LAS TRES PORCIONES en un solo formulario.

    Un solo botón "Mermar" y no uno por porción, y el criterio es la cabeza
    del operario y no la forma del código: **para el que merma es la misma
    acción** —agarra algo que se pudrió y lo tira—, y que una mercadería
    tenga lote y la otra no es un detalle interno. Con dos botones tendría
    que acordarse de cuál usar según qué está tirando, y la mitad de las
    veces usaría el equivocado o no cargaría nada.

    Lo único que cambia según la porción es el selector de LOTE, que existe
    solo para los sueltos: las cajas de una ficha y el pool de segunda no
    tienen lotes que elegir. Es una condición, no media pantalla.
    """
    articulo_elegido = None
    lotes: list[dict] = []
    try:
        articulos = listar_articulos()
        fichas = _fichas_por_articulo()
...
async def cargar_merma_stock_ruta(
    ...
    `que_merma` es "sueltos", "segunda" o el id de una ficha — la misma
    traducción que el Stock Físico, en `_porcion_elegida`, para que las dos
    pantallas no puedan entender distinto la misma pregunta.

    Y ESCRIBE EN DOS TABLAS SEGÚN LA PORCIÓN, que es lo único de todo esto
    que el operario no ve: sueltos y cajas de ficha son `movimientos_stock`
    (con `ficha_id` o sin él); la segunda es una salida de `remitos_segunda`
    con destino 'merma', porque el pool de segunda no está en el stock
    normal — meterla ahí restaría de dos pilas a la vez.

    El LOTE solo se ofrece para los sueltos: las cajas de una ficha y el
    pool no tienen lotes que elegir. Si llega igual para otra porción se
    ignora en vez de rebotar — no cambia lo que se guarda y trabar por algo
    que la pantalla no ofreció es hacerle pagar al operario un detalle
    interno.
    ...
    if not error:
        try:
            ...
            if articulo is not None:
                ficha, es_segunda, error = _porcion_elegida(articulo_id, que_merma)
        except Exception as error_db:
            raise HTTPException(status_code=500, detail=f"Error al conectar con la base de datos: {error_db}") from error_db
        if articulo is None:
            error = "Elegí un artículo válido."

    # El lote es SOLO de los sueltos: para las otras dos porciones ni se
    # ofrece ni se mira.
    lote_tipo, lote_origen_id, lote_etiqueta = None, None, None
    de_los_sueltos = ficha is None and not es_segunda
    if not error and de_los_sueltos and lote.strip():
        tipo, _, origen = lote.partition(":")
        if tipo not in TIPOS_LOTE_STOCK or not origen.isdigit():
            error = "Ese lote no es válido: elegí uno de la lista o dejá el más viejo."
        else:
    ...
    if ficha:
        ...
        que = "bultos de segunda"
    else:
        que = "bultos sueltos"
    aviso = (f"Merma guardada: {_formatear_numero(cantidad_valor)} {que} de "
...
def _renderizar_pantalla_stock_fisico_deposito(
    request: Request, *, articulo_id=None, error=None, aviso=None, fecha=None, status_code: int = 200
):
    ...
    try:
        ...
        # Solo si ese día está vacío: buscarlo siempre sería una consulta de
        # más en el caso normal, que es entrar y ver lo de hoy.
        #
        # Devuelve UNA FECHA y nada más, y por eso puede vivir en esta
        # pantalla: acá no puede viajar un número del sistema ni escondido.
        #
        # Y el día vacío que es su PROPIO más cercano no se ofrece a sí mismo:
        # pasa con la tabla vacía del todo, y un botón que lleva a la misma
        # pantalla es peor que no ofrecer nada. Va en la misma expresión que
        # lo calcula, no en un chequeo aparte: separados, el día que alguien
        # toque uno se olvida del otro.
        mas_cercana = fecha_conteo_stock_mas_cercana(dia) if not contexto["contados_hoy"] else None
        contexto["mas_cercana"] = None if mas_cercana == dia else mas_cercana
    except Exception as error_db:
        raise HTTPException(status_code=500, detail=f"Error al conectar con la base de datos: {error_db}") from error_db
...
def ver_stock_fisico_deposito(request: Request, articulo_id: str | None = None,
                              aviso: str | None = None, fecha: str | None = None):
    ...
    return _renderizar_pantalla_stock_fisico_deposito(
        request, articulo_id=articulo_id, aviso=aviso, fecha=dia
    )
...
def cargar_stock_fisico_deposito_ruta(
    ...
    if not error:
        try:
            articulo = obtener_articulo(int(articulo_id)) if articulo_id.strip().isdigit() else None
            if articulo is not None:
                # La MISMA traducción que usa la Merma: una sola copia.
                ficha, es_segunda, error = _porcion_elegida(articulo_id, que_conto)
        except Exception as error_db:
            raise HTTPException(status_code=500, detail=f"Error al conectar con la base de datos: {error_db}") from error_db
        if articulo is None:
            error = "Elegí un artículo válido."
...
def _motivos_de_merma() -> tuple[str, ...]:
    """Los motivos permitidos para CUALQUIER merma, leídos del CHECK del esquema.

    La lista nació para la merma de segunda y desde el 10/09 vale para las
    tres porciones. El CHECK del que sale sigue siendo el de
    `remitos_segunda` porque es la única de las dos tablas que lo tiene: la
    merma de stock guarda el motivo como texto y la pantalla es la que lo
    limita a la lista. NO se copia la lista acá por eso — se lee del mismo
    lugar, así las tres porciones ofrecen exactamente lo mismo y el día que
    la lista cambie cambian las tres juntas.
    ...
    try:
        ...
        if bloque is None:
            logger.warning("No se encontró el CHECK de motivos de merma en el esquema")
            return ()
        return tuple(re.findall(r"'([^']+)'", bloque.group(1)))
    except Exception:
        logger.exception("No se pudieron leer los motivos de merma")
        return ()


# LOS MOTIVOS DE TODA MERMA, leídos del CHECK de la migración y no copiados
# acá: copiados envejecen en silencio el día que alguien agrega uno a la base
# y la pantalla sigue ofreciendo los viejos. Mismo criterio que los siete
# orígenes de reprocesos_consumos.
#
# Valen para las tres porciones desde el 10/09. Se pudo unificar sin arrastrar
# nada porque NUNCA se cargó una merma (medido el 09/09: cero en toda la
# historia), así que no había texto libre viejo que preservar.
MOTIVOS_MERMA = _motivos_de_merma()
