...
| `agregar_lotes_elegidos_del_armado.sql` (Entrega 3: `pedidos_renglones_lotes_elegidos`, de qué lote dijo el que arma que sacó los bultos de un renglón. Guarda SOLO la excepción — sin filas, el reparto es el del FIFO) | ✅ 2026-09-02 (bloque A OK, bloque B OK, 9/9 en el C; tabla vacía y sin pedidos de prueba) | ✅ 2026-09-02 (bloque A OK, bloque B OK, 9/9 en el C; tabla vacía y sin pedidos de prueba) |
| `aceptar_cierre_modelo_viejo_como_origen.sql` (el compensatorio POSITIVO del corte entra al FIFO como lote —las entradas son "movimientos con `cantidad > 0`" y no miran el tipo— y es el MÁS VIEJO del artículo, así que toda guía R lo toca. Sin esto el INSERT del consumo violaba `reprocesos_consumos_origen_check` y el reproceso REVENTABA con un error crudo delante del operario. Se agrega el valor propio en vez de mapearlo a `'ajuste'` a propósito: `reprocesos_consumos` es documento CONGELADO y esas filas dirían "ajuste" para siempre) | ✅ 2026-09-06 (verificado: el check acepta los 7 valores) | ✅ 2026-09-06 (verificado: el check acepta los 7 valores; Palmala no tuvo corte, así que hoy no hay ningún lote de este tipo — va igual para que las dos bases tengan el mismo esquema) |
| `vacios_vale_caducado.sql` (el vale que no se va a cobrar: se cancela lo que se debe SIN tocar el stock, porque los cajones están en el galpón. Dos columnas nuevas —`sena_vale_caducado_el` y `sena_vale_caducado_motivo`— y un CHECK que exige que haya vale y que el motivo no esté vacío. NO reusa `sena_anulada_el`: haría `num_nonnulls = 2` y esas filas desaparecerían del historial, que filtra `= 1`) | ✅ 2026-09-07 (verificado: las 2 columnas y el check) | ✅ 2026-09-07 (verificado: las 2 columnas y el check) |
