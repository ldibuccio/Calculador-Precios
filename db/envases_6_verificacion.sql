select 'envases_6' as QUE_MIGRACION,
 ...
 (select count(*) from pg_constraint
   where conname in ('movimientos_stock_proveedor_solo_devolucion',
                     'movimientos_stock_compra_solo_devolucion')
     and pg_get_constraintdef(oid) like '%IS DISTINCT FROM%')               as guardas_NULL_SAFE_de_2,
...
-- Todo aplicado: 1 · 1 · 4 · 1 · 2 · 1 · 2 · ofensores 0.
--
-- guardas_NULL_SAFE_de_2 mira la DEFINICION: con `=` da 0.
--
-- ERAN TRES: hasta el 17/09 contaba `movimientos_stock.envase_id`, que saco
-- envases_9 (esa caja SE TIRA). Las de la GUIA R no se tocaron.
