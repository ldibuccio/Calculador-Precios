...
comment on table colegas is 'Colegas con los que se prestan cajas, cada uno con su cuenta corriente. NO son los proveedores de Compras (tabla proveedores) ni los del puesto (proveedores_puesto): circuitos separados a propósito, igual que aquéllos entre sí. Un colega que además sea proveedor va a estar escrito dos veces, y está bien: no se suman en ninguna cuenta.';
comment on column colegas.nombre_normalizado is 'nombre en minúsculas, sin acentos ni espacios de más. Lo calcula el código (normalizar_texto) y el UNIQUE solo exige que sea distinto: el plegado está escrito UNA vez, en Python. Un índice que plegara por su cuenta serían DOS reglas, que es el caso de "ruben" al lado de "Rubén".';
comment on column colegas.activo is 'false = ya no se le presta: sale del selector y nada más. Sus movimientos y su saldo quedan intactos.';
...
create table movimientos_envase (
    ...
    -- Con quién es la cuenta. NULL en todo lo que no sea un préstamo entre
    -- colegas, y obligatorio cuando lo es: lo ata el CHECK de más abajo.
    colega_id       bigint references colegas (id),
    ...
    -- Ata la contraparte con el origen, en las DOS direcciones. `is not
    -- distinct from` y no `=`: un CHECK que evalúa NULL PASA, así que el día
    -- que alguien afloje el not null de `origen` la guarda dejaría de
    -- rechazar en silencio. Ver db/colegas_3_la_cuenta.sql.
    constraint movimientos_envase_colega_segun_origen
...
-- ----------------------------------------------------------------------------
-- 9 bis. LISTADOS_COMPRA — "Qué comprar hoy": cuántos cajones salir a buscar
-- ----------------------------------------------------------------------------
...
create table listados_compra_foto_cajas (
    listado_id   bigint not null references listados_compra (id) on delete cascade,
...
create table listados_compra_kilaje (
    listado_id   bigint not null references listados_compra (id) on delete cascade,
    articulo_id  bigint not null references articulos (id),
    kilaje       numeric not null constraint listados_compra_kilaje_positivo_check
                   check (kilaje > 0),
    primary key (listado_id, articulo_id)
);
