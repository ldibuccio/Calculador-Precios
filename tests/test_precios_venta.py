...
def _fila(articulo_id=1, precio_original=None, precio_nuevo=None):
    return {"articulo_id": articulo_id, "precio_original": precio_original, "precio_nuevo": precio_nuevo}
...
def test_precio_editado_genera_una_fila():
    ...
    assert calcular_cambios_de_precios(filas) == [{"articulo_id": 7, "precio": 550.0}]
...
def test_articulo_sin_precio_previo_con_precio_nuevo_genera_alta():
    ...
    assert calcular_cambios_de_precios(filas) == [{"articulo_id": 3, "precio": 800.0}]
...
def test_varios_articulos_mezclados_en_un_mismo_guardado():
    ...
    assert calcular_cambios_de_precios(filas) == [
        {"articulo_id": 2, "precio": 320.0},
        {"articulo_id": 3, "precio": 900.0},
    ]
